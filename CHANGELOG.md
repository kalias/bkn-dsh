...
- Retarget the compatibility series as `compat/dsh-0.1.6-alpha.2/`: re-port the credential-backed Streamable HTTP MCP headers onto the rewritten `packages/mcp/mcp-client` transport, re-base external published-protocol recognition in `packages/typert/generator/src/analyzer.ts` (required: without it the analyzer discovers 0 of the plugin's 10 public Remote methods), restore the write side of ignorable plugin session records (`Session.append` accepts `LogOnlyEventIntent` for non-surface events — the read side is native, but without it stored plugin events refuse session reload), and regenerate the lockfile patch.
